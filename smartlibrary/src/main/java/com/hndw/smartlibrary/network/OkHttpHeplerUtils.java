...
public class OkHttpHeplerUtils {
    private static OkHttpHeplerUtils okHttpUtils = null;
    private static Context context;
    private WeakReference<Context> weakReference;

    public OkHttpHeplerUtils(Context context) {
        weakReference = new WeakReference<>(context);
        this.context = weakReference.get();
    }

    public static OkHttpHeplerUtils getInstance(Context context) {
        if (okHttpUtils == null) {
            //加同步安全
            synchronized (OkHttpHeplerUtils.class) {
                if (okHttpUtils == null) {
                    okHttpUtils = new OkHttpHeplerUtils(context);
                }
            }
        }
        return okHttpUtils;
     ...
     * 参数三：保存文件的文件名
     */
    public static void downloadFile(final Context context, final String url, final String saveDir) {
        Request request = new Request.Builder().url(url).build();
        Call call = getOkHttpClient().newCall(request);
